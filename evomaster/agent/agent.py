...
def copy_agent(agent: BaseAgent, new_agent_name: str | None = None) -> BaseAgent:
    """复制 Agent 实例，创建新的上下文但共享其他配置
    
    创建一个新的 Agent 实例，该实例：
    - 共享 llm, session, tools, skill_registry, config_dir, enable_tools 等配置
    ...
    Examples:
        ...
        >>> copied_agent = copy_agent(original_agent, new_agent_name="agent_copy_1")
        >>> # copied_agent 和 original_agent 共享 llm, session, tools 等
        >>> # 但拥有独立的对话上下文
    """
    # 复制 AgentConfig，特别是 context_config 需要独立
    if agent.config:
        # 使用 model_copy() 创建配置的深拷贝
        new_config = agent.config.model_copy(deep=True)
    else:
        # 如果 agent.config 为 None，创建一个新的默认配置
        # AgentConfig 在文件后面定义，但在运行时已经可用
    ...
    # 如果是 Agent 子类（不是 BaseAgent），需要处理提示词相关的参数
    if agent_class.__name__ == 'Agent':
        # Agent 子类特有的参数
        # 注意：我们不传递 system_prompt_file 和 user_prompt_file，因为：
        # 1. Agent 可能没有存储这些文件路径
        # 2. 我们会在创建后直接复制已加载的提示词内容
    ...
    # 确保上下文相关的状态被重置（虽然初始化时应该已经是 None/0，但为了安全显式设置）
    ...
    # 注意：context_manager 已经在 __init__ 中创建了新的实例，使用新的 context_config
    # 所以不需要额外操作
